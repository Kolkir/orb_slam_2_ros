...
void Node::Init(ros::NodeHandle& node_handle,
  ...
  node_handle_.param<std::string>(name_of_node_ + "/target_frame_id",
  ...
  rendered_image_publisher_ =
      image_transport.advertise(name_of_node_ + "/debug_image", 1);
...
tf2::Transform Node::TransformToTarget(tf2::Transform tf_in,
  ...
  try {
    ...
    geometry_msgs::TransformStamped tf_msg =
        tfBuffer->lookupTransform(frame_in, frame_target, ros::Time(0));
    // Convert to tf2
  ...
  /*
    ...
    ROS_INFO(" * Translation: {%.3f,%.3f,%.3f}",
                    tf_map2orig.getOrigin().x(), tf_map2orig.getOrigin().y(),
    tf_map2orig.getOrigin().z()); ROS_INFO(" * Rotation: {%.3f,%.3f,%.3f}",
    ...
    ROS_INFO(" * Translation: {%.3f,%.3f,%.3f}",
                    tf_orig2target.getOrigin().x(),
    tf_orig2target.getOrigin().y(), tf_orig2target.getOrigin().z()); ROS_INFO("
    * Rotation: {%.3f,%.3f,%.3f}", RAD2DEG(roll), RAD2DEG(pitch), RAD2DEG(yaw));
    ...
    ROS_INFO(" * Translation: {%.3f,%.3f,%.3f}",
                    tf_map2target.getOrigin().x(),
    tf_map2target.getOrigin().y(), tf_map2target.getOrigin().z()); ROS_INFO(" *
    Rotation: {%.3f,%.3f,%.3f}", RAD2DEG(roll), RAD2DEG(pitch), RAD2DEG(yaw));
  */
...
void Node::PublishPositionAsTransform(cv::Mat position) {
  ...
  tf2::Transform tf_map2target = TransformToTarget(
      tf_transform, camera_frame_id_param_, target_frame_id_param_);

  // Make message
  tf2::Stamped<tf2::Transform> tf_map2target_stamped;
  tf_map2target_stamped = tf2::Stamped<tf2::Transform>(
      tf_map2target, current_frame_time_, map_frame_id_param_);
...
void Node::PublishPositionAsPoseStamped(cv::Mat position) {
  ...
  // Make transform from camera frame to target frame
  tf2::Transform tf_position_target = TransformToTarget(
      tf_position, camera_frame_id_param_, target_frame_id_param_);

  // Make message
  tf2::Stamped<tf2::Transform> tf_position_target_stamped;
  tf_position_target_stamped = tf2::Stamped<tf2::Transform>(
      tf_position_target, current_frame_time_, map_frame_id_param_);
...
cv::Mat Node::TransformToMat(tf2::Transform transfrom_tf) {
  ...
  // Coordinate transformation matrix from ros coordinate system to orb
  // coordinate system
  const tf2::Matrix3x3 tf_orb_to_ros(0, 0, 1, -1, 0, 0, 0, -1, 0);
...
void Node::ParamsChangedCallback(
    orb_slam2_ros::dynamic_reconfigureConfig& config,
    uint32_t level) {
...
void Node::AddOdometry(const tf2::Transform& tf_position, double timestamp) {
  ...
  tf2::Transform tf_target2map = TransformToTarget(
      tf_position, target_frame_id_param_, camera_frame_id_param_);
