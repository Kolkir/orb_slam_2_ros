...
bool LoopClosing::DetectLoop() {
  ...
  if (mvpEnoughConsistentCandidates.empty()) {
    cout << "FAILED: Loop detection ConsistentCandidates = "
         << mvpEnoughConsistentCandidates.size()
         << " KF candidates = " << vpCandidateKFs.size() << std::endl;
    cout << "FAILED: Loop detection BoW min score = " << minScore << std::endl;
    cout << "FAILED: Loop detection MeanCovisibilityConsistency = "
         << meanCovisibilityConsistency << endl;

    mpCurrentKF->SetErase();
    return false;
  } else {
    cout << "SUCCESS: Loop detection ConsistentCandidates = "
         << mvpEnoughConsistentCandidates.size()
         << " KF candidates = " << vpCandidateKFs.size() << std::endl;
    cout << "SUCCESS: Loop detection BoW min score = " << minScore << std::endl;
    cout << "SUCCESS: Loop detection MeanCovisibilityConsistency = "
         << meanCovisibilityConsistency << endl;
    return true;
  }
}
...
bool LoopClosing::ComputeSim3() {
  ...
  const int ransacMinInliers =
      10;                      // 20 should be less then nBoWMatches or equal
  const int nBoWMatches = 10;  // 20 7
  const float orbNNRatio = 0.75f;  // higher less restrictive
  const int nRansacIterations = 5;
  const float sim3OptimizeTh2 =
      20;  // have an influence on inliners count after optimization, bigger
           // less restrictive
  ...
  for (int i = 0; i < nInitialCandidates; i++) {
    ...
    if (nmatches < nBoWMatches) {
      ...
      pSolver->SetRansacParameters(ransacInlinerProbability, ransacMinInliers,
                                   ransacMaxIteration);
  ...
  cout << "Loop closing: ORB match init candidates = " << nInitialCandidates
       << " left after BoW filter = " << nCandidates << endl;
  ...
  while (nCandidates > 0 && !bMatch) {
    for (int i = 0; i < nInitialCandidates; i++) {
      ...
      cv::Mat Scm =
          pSolver->iterate(nRansacIterations, bNoMore, vbInliers, nInliers);
      ...
      if (!Scm.empty()) {
        ...
        const int nInliers =
            Optimizer::OptimizeSim3(mpCurrentKF, pKF, vpMapPointMatches, gScm,
                                    sim3OptimizeTh2, mbFixScale);

        cout << "Loop closing OptimizeSim3 inliners left = " << nInliers
             << endl;
  ...
  if (!bMatch) {
    cout << "FAILED: Loop closing mean Inliers Num = " << meanInliersNum
         << endl;
    for (int i = 0; i < nInitialCandidates; i++)
  ...
  matcher.SearchByProjection(mpCurrentKF, mScw, mvpLoopMapPoints,
                             mvpCurrentMatchedPoints, radiusScaleFactor);
  ...
  if (nTotalMatches >= 40) {
    cout << "SUCCESS: Loop closing Total Matches(lim 40) = " << nTotalMatches
         << endl;
    for (int i = 0; i < nInitialCandidates; i++)
    ...
    cout << "FAILED: Loop closing Total Matches(lim 40) = " << nTotalMatches
         << endl;
    for (int i = 0; i < nInitialCandidates; i++)
      mvpEnoughConsistentCandidates[i]->SetErase();
    mpCurrentKF->SetErase();
