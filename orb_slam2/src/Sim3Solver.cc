...
Sim3Solver::Sim3Solver(KeyFrame* pKF1,
  ...
  double mvnMaxErrorFactor =
      9.210;  // used to define the error for selecting inliners in RANSAC
  ...
  if (cv::countNonZero(kf1_odom) > 0 && cv::countNonZero(kf2_odom) > 0) {
    ...
    cout << "Sim3Solver KF1 odom error = " << kf1_error
         << " KF2 odom error = " << kf2_error
         << " error scale factor = " << mvnMaxErrorFactor << endl;
  }
...
void Sim3Solver::SetRansacParameters(double probability,
  ...
  cout << "Sim3Solver RansacMaxIts = " << mRansacMaxIts << std::endl;
...
cv::Mat Sim3Solver::iterate(int nIterations,
  ...
  while (mnIterations < mRansacMaxIts && nCurrentIterations < nIterations) {
    ...
    if (mnInliersi >= mnBestInliers) {
      cout << "Sim3Solver inliners num = " << mnInliersi
           << " best num = " << mnBestInliers
           << " RansacMinInliers = " << mRansacMinInliers << endl;
      ...
      cout << "FAILED: Sim3Solver inliners num = " << mnInliersi
           << " best num = " << mnBestInliers
           << " RansacMinInliers = " << mRansacMinInliers << endl;
    }
  }
...
void Sim3Solver::CheckInliers() {
  ...
  cout << "Sim3Solver::CheckInliers inliner candidate num = " << mvP1im1.size()
       << endl;
  ...
  for (size_t i = 0; i < mvP1im1.size(); i++) {
    ...
    if (err1 < mvnMaxError1[i] && err2 < mvnMaxError2[i]) {
      ...
      cout << "Sim3Solver::CheckInliers err1 = " << err1
           << " limit = " << mvnMaxError1[i] << endl;
      cout << "Sim3Solver::CheckInliers err2 = " << err2
           << " limit = " << mvnMaxError2[i] << endl;
      mvbInliersi[i] = false;
    }
  }
}
