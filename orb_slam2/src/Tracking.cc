...
void Tracking::AddOdometryToCurrentFrame() {
  ...
  if (odom_iter == mOdometryPoses.end()) {
    odom_iter = mOdometryPoses.lower_bound(mCurrentFrame.mTimeStamp -
                                           odomTimeTolerance);
  }
  ...
  if (odom_iter != mOdometryPoses.end()) {
    ...
    std::cerr << "Failed to find corresponding odometry pose for frame at "
              << mCurrentFrame.mTimeStamp
              << " sec timestamp using the last one\n";
...
void Tracking::MonocularInitialization() {
  if (!mpInitializer) {
    ...
    if (nmatches < min_init_obj_num) {
      delete mpInitializer;
      mpInitializer = static_cast<Initializer*>(NULL);
      std::cout << "Tracking::MonocularInitialization failed to find enough "
                   "correspondences "
                << nmatches << std::endl;
      return;
    }
    std::cout << "Tracking::MonocularInitialization number of correspondences "
                 "found = "
              << nmatches << std::endl;
